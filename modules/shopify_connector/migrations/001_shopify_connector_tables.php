<?php
/**
 * c:\wamp64\www\perfex_crm\modules\shopify_connector\migrations\001_shopify_connector_tables.php
 */
defined('BASEPATH') or exit('No direct script access allowed');

class Migration_Shopify_connector_tables extends App_module_migration
{
    public function up()
    {
        $CI = &get_instance();
        ...
        // Alter tables if they already exist without these columns
        if ($CI->db->table_exists(db_prefix() . 'shopify_stores')) {
            ...
            if ($CI->db->field_exists('warehouse_location_id', db_prefix() . 'shopify_stores')) {
                // Shopify location IDs are big integers, better store them as VARCHAR(50)
    ...
    public function down()
    {
        ...
        // Drop tables in reverse order to avoid foreign key issues (even though we didn't specify explicit FK constraints in MySQL, it's a good practice)
        $CI->db->query("DROP TABLE IF EXISTS `" . db_prefix() . "shopify_stock_reservations`;");
        $CI->db->query("DROP TABLE IF EXISTS `" . db_prefix() . "shopify_integration_logs`;");
        $CI->db->query("DROP TABLE IF EXISTS `" . db_prefix() . "shopify_accounting_entries`;");
        $CI->db->query("DROP TABLE IF EXISTS `" . db_prefix() . "shopify_inventory_sync`;");
        $CI->db->query("DROP TABLE IF EXISTS `" . db_prefix() . "shopify_fulfillment_updates`;");
        $CI->db->query("DROP TABLE IF EXISTS `" . db_prefix() . "shopify_webhook_events`;");
        $CI->db->query("DROP TABLE IF EXISTS `" . db_prefix() . "shopify_webhooks`;");
        $CI->db->query("DROP TABLE IF EXISTS `" . db_prefix() . "shopify_product_mappings`;");
        $CI->db->query("DROP TABLE IF EXISTS `" . db_prefix() . "shopify_order_items`;");
        $CI->db->query("DROP TABLE IF EXISTS `" . db_prefix() . "shopify_orders`;");
        $CI->db->query("DROP TABLE IF EXISTS `" . db_prefix() . "shopify_stores`;");
    }
}
