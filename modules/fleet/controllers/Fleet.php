...
class Fleet extends AdminController
{
    ...
    */
    ...
    public function fuels(){
        ...
        // Pass open/recent assignments for trip-type linkage in the fuel form
        $data['assignments'] = $this->db
            ->select('a.id, a.vehicle_id, a.driver_id, a.start_time, a.starting_odometer,
                      CONCAT(s.firstname," ",s.lastname) AS driver_name,
                      v.name AS vehicle_name')
            ->from(db_prefix() . 'fleet_vehicle_assignments a')
            ->join(db_prefix() . 'staff s',         's.staffid = a.driver_id',   'left')
            ->join(db_prefix() . 'fleet_vehicles v', 'v.id = a.vehicle_id',       'left')
            ->where('a.end_time IS NULL OR a.end_time >= DATE_SUB(NOW(), INTERVAL 7 DAY)', null, false)
            ->order_by('a.id', 'DESC')
            ->limit(50)
            ->get()->result();
    ...
    public function fuel_history_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $select = [
                ...
                'fuel_time',
                'vendor_id',
                db_prefix() . 'fleet_fuel_history.trip_type as trip_type',
                db_prefix() . 'fleet_fuel_history.odometer as odometer',
                db_prefix() . 'fleet_fuel_history.odometer_after as odometer_after',
                'gallons',
                'price',
            ];
            ...
            $trip_labels = [
                'pre_trip'  => '<span class="label label-primary">Pre-Trip</span>',
                'post_trip' => '<span class="label label-warning">Post-Trip</span>',
            ...
            foreach ($rResult as $aRow) {
                ...
                $tripType   = $aRow['trip_type'] ?? 'regular';
                $row[] = $categoryOutput;
                $row[] = _dt($aRow['fuel_time']);
                $row[] = get_vendor_company_name($aRow['vendor_id']);
                $row[] = $trip_labels[$tripType] ?? $trip_labels['regular'];
                $row[] = $aRow['odometer'] != null ? number_format($aRow['odometer']) : '—';
                $row[] = $aRow['odometer_after'] != null ? number_format($aRow['odometer_after']) : '—';
    ...
    public function add_fuel(){
        ...
        // Sanitise new fields
        $data['trip_type']      = in_array($data['trip_type'] ?? '', ['pre_trip','post_trip','regular'])
                                    ? $data['trip_type'] : 'regular';
        $data['assignment_id']  = !empty($data['assignment_id']) ? (int)$data['assignment_id'] : null;
        $data['odometer_after'] = !empty($data['odometer_after']) ? (int)$data['odometer_after'] : null;
        ...
        if ($data['id'] == '') {
            ...
            if ($success) {
                // If pre-trip, sync starting_odometer on the assignment
                if ($data['trip_type'] === 'pre_trip' && $data['assignment_id']) {
                    $this->db->where('id', $data['assignment_id'])
                             ->update(db_prefix() . 'fleet_vehicle_assignments',
                                      ['starting_odometer' => $data['odometer'] ?? null]);
                }
                // If post-trip, sync ending_odometer on the assignment
                if ($data['trip_type'] === 'post_trip' && $data['assignment_id'] && $data['odometer_after']) {
                    $this->db->where('id', $data['assignment_id'])
                             ->update(db_prefix() . 'fleet_vehicle_assignments',
                                      ['ending_odometer' => $data['odometer_after']]);
                }
                $message = _l('added_successfully');
            } else {
    ...
    */
    ...
    public function required_module() {
        // No hard module requirements — purchase and hr_profile are optional integrations.
        // Fleet runs standalone; trip booking links to courier; driver training links to xetuu_hr.
    }

    public function required_module_detail() {
        $data = [];
        $data['optional'] = [
            'purchase' => fleet_get_status_modules('purchase') ? 1 : 0,
            'xetuu_hr' => fleet_get_status_modules('xetuu_hr') ? 1 : 0,
            'courier'  => fleet_get_status_modules('courier')  ? 1 : 0,
        ];
