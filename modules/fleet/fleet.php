...
function fleet_module_init_menu_items() {
    ...
    // Auto-create trip booking tables if they were added after initial module activation.
    if (!$CI->db->table_exists(db_prefix() . 'fleet_trips')) {
        $CI->db->query('CREATE TABLE IF NOT EXISTS `' . db_prefix() . "fleet_trips` (
    ...
    if (!$CI->db->table_exists(db_prefix() . 'fleet_trip_offloading')) {
        $CI->db->query('CREATE TABLE IF NOT EXISTS `' . db_prefix() . "fleet_trip_offloading` (
            `id`                  INT(11)       NOT NULL AUTO_INCREMENT,
    ...
    // Migrate existing fleet_trips — add columns added in later releases.
    if ($CI->db->table_exists(db_prefix() . 'fleet_trips')) {
        $fields_to_add = [
            'fuel_request_id' => 'INT(11) NULL DEFAULT NULL',
            'start_odometer'  => 'DECIMAL(10,2) NULL DEFAULT NULL',
        ...
        foreach ($fields_to_add as $field => $def) {
            if (!$CI->db->field_exists($field, db_prefix() . 'fleet_trips')) {
                $CI->db->query('ALTER TABLE `' . db_prefix() . 'fleet_trips` ADD COLUMN `' . $field . '` ' . $def);
            }
        }
    }
