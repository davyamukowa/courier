...
class Pickup_model extends App_Model
{
    ...
    public function get($return_count = false, $status = null, $staff_id = null, $role = null, $country_id = null): array|bool|string
    {
        ...
        $this->db->join(db_prefix() . '_pickup_contacts c', 'p.contact_person_id = c.id', 'left');
        $this->db->join(db_prefix() . 'countries ct', 'p.country_id = ct.country_id', 'left');
        ...
        if ($country_id !== null) {
            $this->db->where('p.staff_country_id', (int)$country_id);
        }
