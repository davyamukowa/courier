...
class Shipment_model extends App_Model
{
    ...
    public function get_shipment_count_by_status($status_id = null, $staff_id = null, $country_id = null)
    {
        $this->db->select('s.*');
        ...
        if ($country_id !== null) {
            $this->db->where('s.country_id', (int)$country_id);
        }
    ...
    public function get_shipments_details($staff_id = null, $type = null, $mode = null, $mode_type = null, $country_id = null)
    {
        // Fetch the shipment details
        $this->db->select('s.id,(SELECT p.id FROM ' . db_prefix() . '_pickups p WHERE p.shipment_id = s.id LIMIT 1) AS pickup_id, s.tracking_id, s.waybill_number, s.shipping_mode, s.invoice_id, s.status_id, s.commercial_invoice_url, s.created_at, ss.status_name, ss.description as status_description, s.sender_id, s.company_id, s.recipient_id, s.recipient_company_id, s.is_portal_request, s.quoted_amount', FALSE);
        $this->db->from(db_prefix() . '_shipments s');
    ...
    public function filter_shipment_details($staff_id = null, $status_id = null, $filter_staff_id = null, $startDate = null, $endDate = null, $type = null, $mode = null, $mode_type = null, $country_id = null)
    {
        // Fetch the shipment details
        $this->db->select('s.id, s.tracking_id, s.waybill_number, s.shipping_mode, s.invoice_id, s.status_id, s.created_at, s.fcl_shipment, ss.status_name, ss.description as status_description, s.sender_id, s.company_id, s.recipient_id, s.recipient_company_id', FALSE);
        $this->db->from(db_prefix() . '_shipments s');
        $this->db->join(db_prefix() . '_shipment_statuses ss', 'ss.id = s.status_id', 'left');
