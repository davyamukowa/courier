...
class Shipments extends AdminController
{
    ...
    public function main()
    {
        ...
        $country_id = $this->get_staff_country_id();
        ...
        if ($country_id)  { $this->db->where('s.country_id', (int)$country_id); }
        ...
        switch ($group) {
            ...
            case 'create_shipment':
                ...
                redirect('admin/courier/shipments/create' . ($qs ? '?' . $qs : ''));
                return;
            ...
            case 'list_shipments':
                ...
                redirect('admin/courier/shipments/index' . ($qs ? '?' . $qs : ''));
                return;
    ...
    public function index()
    {
        ...
        if ($has_active_filter && !empty($this->session->userdata('shipment_details'))) {
            ...
            $data['shipment_details'] = $this->Shipment_model->filter_shipment_details(
                $staff_param, $sess_status, null, null, null, $type, null, null, $country_id
            );
        } else {
            if (staff_can('view_all_shipments', 'courier-shipments')) {
                $data['shipment_details'] = $this->Shipment_model->get_shipments_details(null, $type, $mode, $mode_type, $country_id);
            } else {
                $data['shipment_details'] = $this->Shipment_model->get_shipments_details($staff_id, $type, $mode, $mode_type, $country_id);
            }
        }
...
// New private method to add recipient
    ...
    public function store_pickup_data($shipment_id)
    {
        if ($this->input->post('hasPickup') !== null) {
            ...
            $pickup_data = [
                ...
                'staff_country_id'  => $this->get_staff_country_id(),
    ...
    private function _assert_ownership($details)
    {
        ...
        if ((int)$details['shipment']->staff_id !== (int)get_staff_user_id()) {
            ...
            redirect('admin/courier/shipments/main');
            return false;
        }
        return true;
    }
    ...
    public function send_waybill_email($id)
    {
        ...
        $tracking_url  = base_url('courier/tracking') . '?track=' . urlencode($shipment->waybill_number ?? $shipment->tracking_id);
    ...
    /**
     * Return the country_id assigned to the currently logged-in staff member
     * via the courier country isolation system, or NULL if none / if admin.
     */
    private function get_staff_country_id()
    {
        if (is_admin()) {
            return null;
        }
