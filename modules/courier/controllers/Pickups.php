...
class Pickups extends AdminController
{
    ...
    public function main()
    {
        ...
        foreach ($statuses as $status) {
            if ($user_role === 'Fleet: Driver') {
                ...
                $pickup_counts[$status] = $this->Pickup_model->get(true, $status, null, null, $country_id);
            } else {
                $pickup_counts[$status] = $this->Pickup_model->get(true, $status, $staff_id, null, $country_id);
            }
        }
    ...
    public function index()
    {
        ...
        if (staff_can('view_all_pickups', 'courier-pickups')) {
            $data['pickups'] = $this->Pickup_model->get(false, null, null, null, $country_id);
        } elseif ($user_role === 'Fleet: Driver') {
            $data['pickups'] = $this->Pickup_model->get(false, null, $staff_id, 'driver');
        } else {
            $data['pickups'] = $this->Pickup_model->get(false, null, $staff_id, null, $country_id);
        }
