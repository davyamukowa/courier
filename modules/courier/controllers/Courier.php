...
class Courier extends AdminController
{
    ...
    public function dashboard()
    {
        ...
        $data['total_shipments'] = $can_all
            ? $this->Shipment_model->get_shipment_count_by_status(null, null, $country_id)
            : $this->Shipment_model->get_shipment_count_by_status(null, $staff_id, $country_id);

        // Pending pickups
        $can_pickups = staff_can('view_all_pickups', 'courier-pickups');
        $data['total_pickups'] = $can_pickups
            ? $this->Pickup_model->get(true, null, null, null, $country_id)
            : $this->Pickup_model->get(true, null, $staff_id, null, $country_id);

        $data['pending_pickups'] = $can_pickups
            ? $this->Pickup_model->get(true, 'pending', null, null, $country_id)
            : $this->Pickup_model->get(true, 'pending', $staff_id, null, $country_id);
    ...
    /**
     * AJAX: return the courier country assigned to a staff member.
     * Used by the JS injected into the staff profile form.
     */
    public function get_staff_country()
    {
        if (!is_staff_logged_in()) {
            echo json_encode(['country_id' => 0, 'country_name' => '']);
            return;
        }
        ...
        $row = $this->db->where('staff_id', $staff_id)
                        ...
                        ->row();
        echo json_encode([
            'country_id'   => $row ? (int)$row->country_id   : 0,
            'country_name' => $row ? $row->country_name : '',
        ]);
    }
