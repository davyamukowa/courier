...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var POD_SP_JSON_URL = '<?php echo admin_url("courier/settings/service_points_json"); ?>';
        ...
        var STATES_URL = '<?php echo admin_url("courier/states"); ?>';
                                        ...
                                        <div style="padding-left:15px; padding-right:15px;" class="row">
                                            ...
                                            <div class="col-md-6 col-sm-12">
                                                <div class="custom-form-group">
                                                    ...
                                                    <input type="hidden" name="courier_company_id" value="0"/>
                                                    <?php
                                                    /* Resolve display value — multiple fallbacks */
                                                    $_lc_disp = '';
                                                    if (!empty($courier_logistic_company) && $courier_logistic_company !== 'GO Shipping') {
                                                        $_lc_disp = $courier_logistic_company;
                                                    } elseif (!empty(get_option('courier_logistic_company')) && get_option('courier_logistic_company') !== 'GO Shipping') {
                                                        $_lc_disp = get_option('courier_logistic_company');
                                                    } elseif (!empty(get_option('companyname'))) {
                                                        $_lc_disp = get_option('companyname');
                                                    }
                                                    ?>
                                                    <div style="display:flex; align-items:center; gap:6px;">
                                                        <input type="text"
                                                               id="logistic_company_display"
                                                               class="custom-input"
                                                               readonly
                                                               style="background:#f0fff4; border:1.5px solid #28a745; color:#155724; font-weight:600; cursor:default; flex:1; min-width:0;"
                                                               value="<?php echo htmlspecialchars($_lc_disp); ?>"
                                                               placeholder="Click + to set company name">
                                                        <?php if(is_admin()): ?>
                                                        <button type="button"
                                                                onclick="openLogisticModal()"
                                                                title="Add / Change Logistic Company"
                                                                style="background:linear-gradient(135deg,#2e7d32,#1b5e20);color:#fff;border:none;border-radius:6px;padding:7px 11px;cursor:pointer;font-size:13px;font-weight:700;flex-shrink:0;line-height:1;transition:opacity .18s;">
...
function saveLogisticCompany() {
    ...
    $.ajax({
        url: '<?php echo admin_url("courier/shipments/save_logistic_company"); ?>',
        type: 'POST',
        data: {
