...
<div id="wrapper">
    <div class="content">
        <div class="row">
            ...
            $wb_list_url  = 'courier/shipments?type=' . $wb_type;
            if ($wb_mode) {
            ...
            <div class="col-md-3">
                <div class="courier-sidebar">
                    ...
                    <nav class="courier-sidebar-nav">
                        ...
                        $ci_url = !empty($shipment_details['shipment']->commercial_invoice_url)
                            ...
                            : admin_url('courier/shipments/commercial_invoice/' . $wb_sid);
                        ...
                        <div style="padding:6px 10px; display:flex; flex-direction:column; gap:6px;">
                            <a href="<?php echo admin_url('courier/shipments/courier_invoice/' . $wb_sid); ?>"
                            ...
                            <?php if (is_admin()): ?>
                            <a href="<?php echo admin_url('courier/shipments/consignment_note/' . $wb_sid); ?>"
                               style="display:flex; align-items:center; gap:8px; background:linear-gradient(135deg,#2e7d32,#1b5e20); color:#fff; padding:8px 14px; border-radius:6px; font-size:13px; font-weight:600; text-decoration:none; transition:opacity .2s;"
                               onmouseover="this.style.opacity='.85'" onmouseout="this.style.opacity='1'">
                        ...
                        <?php if (is_admin()): ?>
                        <div class="courier-nav-divider"></div>
                        <a href="<?php echo admin_url('courier/shipments/create?type=' . $wb_type); ?>"
                           class="courier-nav-item">
                ...
                <div class="courier-sidebar" style="margin-top:12px;">
                    ...
                    <div style="padding:10px 14px; font-size:12px; line-height:1.8; color:#444;">
                        ...
                        <strong>Created:</strong> <?php echo date('d M Y', strtotime($shipment_details['shipment']->created_at)); ?>
                    </div>
                </div>
            </div>
            <!-- ── End left sidebar ────────────────────────────────── -->
...
$('#wb_cpm_submit').on('click', function () {
    ...
    $.ajax({
        url:      '<?php echo admin_url("courier/shipments/confirm_portal_request/" . $_wb_sid); ?>',
        type:     'POST',
        dataType: 'json',
        data: {
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        if (file_exists(module_dir_path('fleet'))) {
                            $CI = &get_instance();
                            $CI->load->model('fleet/Fleet_trips_model');
                            $linked_trips = $CI->Fleet_trips_model->get_all(['shipment_id' => $wb_sid]);
                        }
                        ?>
                        <?php if (!empty($linked_trips)): ?>
                        <div style="margin-top:20px;" class="row">
                            <div class="col-md-12">
                                <h4 style="font-size:16px; font-weight:600;"><i class="fa fa-truck"></i> Linked Fleet Trips</h4>
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped">
                                        <thead>
                                        <tr>
                                        ...
                                        <tbody>
                                        <?php foreach ($linked_trips as $t): ?>
                                        <tr>
                                            <td>#<?php echo $t->id; ?></td>
                                            <td><?php echo _dt($t->trip_date); ?></td>
                                            <td><?php echo htmlspecialchars(($t->from_point_name ?? 'Origin') . ' -> ' . ($t->to_point_name ?? 'Destination')); ?></td>
                                            <td><?php echo htmlspecialchars($t->vehicle_name); ?></td>
                                            <td><?php echo htmlspecialchars($t->driver_name ?? '-'); ?></td>
                                            <td><span class="label label-info"><?php echo ucfirst(str_replace('_', ' ', $t->status)); ?></span></td>
...
$('#send_waybill_email_btn').on('click', function () {
    ...
    $.ajax({
        url: '<?php echo admin_url('courier/shipments/send_waybill_email/' . $shipment_details['shipment']->id); ?>',
        type: 'POST',
        dataType: 'json',
        data: { email: email },
