...
<div class="content">
    ...
    <div class="row tw-mb-4">
        ...
        <div class="col-md-4 tw-text-right">
            <a href="<?php echo admin_url('courier/agents/edit/' . $agent->id); ?>" class="btn btn-info btn-sm">
                <i class="fa fa-pencil"></i> Edit
            </a>
            <button class="btn btn-default btn-sm" data-toggle="modal" data-target="#resetPasswordModal">
                <i class="fa fa-key"></i> Reset Password
            </button>
            <?php if ($is_suspended): ?>
                <a href="<?php echo admin_url('courier/agents/activate/' . $agent->id); ?>" class="btn btn-success btn-sm" onclick="return confirm('Activate this agent?');">
            ...
            <?php endif; ?>
            <?php if (is_admin()): ?>
                <a href="<?php echo admin_url('courier/agents/delete/' . $agent->id); ?>" class="btn btn-danger btn-sm" onclick="return confirm('Permanently delete this agent?');">
    ...
    <div class="row tw-mb-4">
        <div class="col-md-3">
            <div class="panel panel-default tw-text-center tw-py-3">
                <div style="font-size:28px;font-weight:bold;"><?php echo number_format($stats['total_shipments']); ?></div>
                <div class="text-muted">Total Shipments</div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="panel panel-default tw-text-center tw-py-3">
                <div style="font-size:28px;font-weight:bold;"><?php echo number_format($stats['total_revenue'], 2); ?></div>
                <div class="text-muted">Total Revenue</div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="panel panel-default tw-text-center tw-py-3">
                <div style="font-size:28px;font-weight:bold;"><?php echo number_format($stats['total_paid'], 2); ?></div>
                <div class="text-muted">Total Paid</div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="panel panel-default tw-text-center tw-py-3">
                <div style="font-size:28px;font-weight:bold;"><?php echo number_format($stats['outstanding'], 2); ?></div>
    ...
    <!-- Tabs -->
    <style>
    .agent-tabs { display:flex; gap:4px; flex-wrap:wrap; background:#f5c518; padding:8px 12px; border-radius:8px; margin-bottom:0; list-style:none; border:none; }
    .agent-tabs > li > a {
        display:inline-flex; align-items:center; gap:6px;
        padding:9px 18px; border-radius:6px;
        font-weight:700; font-size:13.5px; color:#333;
        background:transparent; border:none;
        text-decoration:none; transition:background .15s;
    }
    .agent-tabs > li > a:hover { background:rgba(0,0,0,.1); color:#111; text-decoration:none; }
    .agent-tabs > li.active > a { background:#fff; color:#222; box-shadow:0 2px 6px rgba(0,0,0,.18); }
    .tab-content.agent-tab-content { background:#fff; border:1px solid #e0e0e0; border-radius:0 0 8px 8px; padding:24px; }
    </style>

    <ul class="agent-tabs" role="tablist">
        <li role="presentation" class="active"><a href="#tab-overview"     data-toggle="tab"><i class="fa fa-user"></i> Overview</a></li>
        <li role="presentation"><a href="#tab-permissions" data-toggle="tab"><i class="fa fa-shield"></i> Permissions</a></li>
        <li role="presentation"><a href="#tab-shipments"   data-toggle="tab"><i class="fa fa-truck"></i> Shipments</a></li>
        <li role="presentation"><a href="#tab-invoices"    data-toggle="tab"><i class="fa fa-file-text"></i> Invoices</a></li>
        <li role="presentation"><a href="#tab-documents"   data-toggle="tab"><i class="fa fa-folder"></i> Documents</a></li>
        <li role="presentation"><a href="#tab-notes"       data-toggle="tab"><i class="fa fa-pencil-square-o"></i> Admin Notes</a></li>
    </ul>
            ...
            <table class="table table-striped">
                ...
                <?php foreach ($shipments as $s): ?>
                    <tr>
                        ...
                        <td><a href="<?php echo admin_url('courier/shipments/waybill/' . $s->id); ?>" class="btn btn-xs btn-default"><i class="fa fa-eye"></i></a></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
            ...
            <table class="table table-striped">
                ...
                <?php foreach ($invoices as $inv): ?>
                    ...
                    <tr>
                        ...
                        <td><a href="<?php echo admin_url('courier/shipments/courier_invoice/' . $inv->id); ?>" class="btn btn-xs btn-default"><i class="fa fa-file-text"></i></a></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
...
$(function () {
    $('#btn-save-permissions').on('click', function () {
        ...
        $.ajax({
            url:         '<?php echo admin_url('courier/agents/save_permissions/' . $agent->id); ?>',
            type:        'POST',
            data:        postData,
            dataType:    'json',
