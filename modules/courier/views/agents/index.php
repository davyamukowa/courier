...
<div class="tw-flex tw-justify-between tw-items-center tw-mb-4">
    <div></div>
    <?php if (is_admin()): ?>
    <button id="btn-reset-role" class="btn btn-warning">
...
<table class="table dt-table" id="agents-table">
    <thead>
    <tr>
        <th>Agent Number</th>
        <th>Agent</th>
    ...
    <?php foreach ($agents as $agent): ?>
        ...
        <tr>
            ...
            <td class="agent-actions">
                <a href="<?php echo admin_url('courier/agents/view/' . $agent->id); ?>" class="btn-view">
                    <i class="fa fa-eye"></i> View
                </a>
                <a href="<?php echo admin_url('courier/agents/edit/' . $agent->id); ?>" class="btn-edit">
                    <i class="fa fa-pencil"></i> Edit
                </a>
                <?php if ($is_suspended): ?>
                    <a href="<?php echo admin_url('courier/agents/activate/' . $agent->id); ?>" class="btn-activate" onclick="return confirm('Activate this agent?');">
                ...
                <?php endif; ?>
                <?php if (is_admin()): ?>
                    <a href="<?php echo admin_url('courier/agents/delete/' . $agent->id); ?>" class="btn-delete" onclick="return confirm('Permanently delete this agent?');">
...
<div style="text-align:center;padding:60px 0;color:#aaa;">
    ...
    No agents yet. <a href="<?php echo admin_url('courier/agents/main?group=create_agent'); ?>">Create the first agent</a>.
</div>
<?php endif; ?>
        ...
        <form method="POST" id="suspend-form" action="">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-warning"><i class="fa fa-ban"></i> Suspend Agent</button>
                </div>
            </div>
        </form>
    </div>
...
$(function () {
    $('.btn-do-suspend').on('click', function () {
        ...
        $('#suspend-form').attr('action', '<?php echo admin_url('courier/agents/suspend/'); ?>' + $(this).data('id'));
    ...
    $('#btn-reset-role').on('click', function () {
        ...
        $.post('<?php echo admin_url('courier/agents/reset_permissions'); ?>', {
