...
// Notification bridge
require __DIR__ . '/includes/client_tenant_bridge_notification.php'
?>
...
if (SAAS_IS_MAGIC_AUTH && !SINGLE_PORTAL_PARENT && !SINGLE_PORTAL_MESSAGE.length && SINGLE_PORTAL_TARGET_ORIGIN) {
    ...
    let saasShouldNotRedirectOrphanedPage = (SAAS_SINGLE_PORTAL_TARGET_IS_CUSTOM_DOMAIN && !
        ...
        SAAS_SINGLE_PORTAL_REDIRECTION_COUNT > 5;
    ...
    if (saasShouldNotRedirectOrphanedPage) {
        ...
        SaasSinglePortalNotificationBrigde.saveNotes();
...
if (SINGLE_PORTAL_PARENT) {
    ...
    // Alert any cached notifications
    SaasSinglePortalNotificationBrigde.alertSavedNotes();

    // Function to handle navigation attempts
    function SaaSHandleCrossOriginNavigation(event, url = '', source = '') {
    ...
    document.addEventListener('click', (event) => {
        if (event.target.tagName === 'A' || event.target.tagName === 'FORM') {
            SaaSHandleCrossOriginNavigation(event, '', 'click');
        }
    });
