...
$total_recurring_invoices = $CI->db->where(perfex_saas_column('packageid') . ' >', 0)->where('recurring', 1)->count_all_results(db_prefix() . 'invoices');
$total_subscriptions = $CI->db->select('COUNT(DISTINCT(subscription_id)) as total')
    ->join(db_prefix() . 'subscriptions', 'subscription_id = ' . db_prefix() . 'subscriptions.id', 'INNER')
    ->where(perfex_saas_column('packageid') . ' >', 0)
    ->where('subscription_id' . ' >', 0)
    ->where(db_prefix() . 'subscriptions.status', 'active')
    ->where(db_prefix() . 'invoices.status', Invoices_model::STATUS_PAID)
    ->where('recurring', 0)
    ->get(db_prefix() . 'invoices')->row()->total ?? 0;
$total_subscriptions = $total_recurring_invoices + $total_subscriptions;
