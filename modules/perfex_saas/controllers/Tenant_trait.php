...
trait Tenant_trait
{
    ...
    public function deployTenants(?int $clientid)
    {
        ...
        // Checking a slug status
        $slugs = (array)$this->input->get('slugs');
        if (!empty($slugs) && $is_status_check == 2) {
            $this->db->where_in('slug', $slugs);
            $companies = $this->perfex_saas_model->companies();
            $response = [];
            foreach ($companies as $key => $company) {
                $response[$company->slug] = ['status' => $company->status, 'completed' => !perfex_saas_company_status_can_deploy($company)];
            }
            return $response;
        }
    ...
    public function subscribeToPackage(int $clientid, string $packageslug)
    {
        try {
            ...
            $package = $this->perfex_saas_model->parse_package($package);
            $package = hooks()->apply_filters('perfex_saas_will_subscribe_to_package', $package);

            if (is_client_logged_in() && $package->is_private == '1') {
                throw new \Exception("404: " . _l('perfex_saas_packages'), 1);
            }
            ...
            if (empty($companies)) {
                ...
                if (get_option('perfex_saas_autocreate_first_company') == '1') {
                    ...
                    if (!empty($data) && !empty($data['clientid'] ?? '')) {
    ...
    public function cancelSubscription(int $clientid)
    {
        try {
            ...
            if ($invoice) {
                ...
                return [
                    'success' => $type != 'immediately' ? _l('perfex_saas_pricing_cancelled_for_renewal') : _l('perfex_saas_pricing_cancelled'),
                    'redirect' => '',
                    'invoice' => $invoice,
                    'package' => $package
                ];
    ...
    private function createOrUpdateCompany(int $clientid, $tenantid = '')
    {
        ...
        try {
            ...
            if (!empty($tenantid)) {
                $data['id'] = $tenantid;
                $company = $this->perfex_saas_model->companies($tenantid);
            } else {
                // Creating new
            ...
            if ($_id) {
                ...
                if (!empty($custom_domain)) {
                    ...
                    $can_send_x_domain_note = true;

                    // check if custom domain change
                    if ($company && $custom_domain == $company->custom_domain) {
                        $can_send_x_domain_note = false;
                    }

                    if ($can_send_x_domain_note) {
                        if (!$company) {
                            $company = $this->perfex_saas_model->companies($_id);
                        }
                        perfex_saas_send_customdomain_request_notice($company, $custom_domain, $invoice);
