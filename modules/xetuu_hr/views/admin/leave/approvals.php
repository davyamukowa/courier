...
.ap-kpi { background:#fff; border-radius:12px; padding:14px 16px;
...
/* Cards */
.ap-card { background:#fff; border-radius:14px;
...
.ap-tag { display:inline-flex;align-items:center;gap:5px;padding:3px 10px;border-radius:6px;font-size:11px;font-weight:700; }

.ap-btn-approve { padding:8px 20px;border-radius:8px;font-size:12px;font-weight:700;
                  background:#16a34a;color:#fff;border:none;cursor:pointer;
                  display:flex;align-items:center;gap:6px;
                  box-shadow:0 2px 8px rgba(22,163,74,.3);transition:transform .15s,box-shadow .15s; }
.ap-btn-approve:hover { transform:translateY(-1px);box-shadow:0 4px 14px rgba(22,163,74,.4); }
...
/* Approver settings panel */
.apv-search { width:100%;padding:6px 10px;border:1px solid #e5e7eb;border-radius:7px;
              font-size:12px;outline:none;margin-bottom:8px;box-sizing:border-box; }
.apv-search:focus { border-color:#d97706;box-shadow:0 0 0 2px rgba(217,119,6,.15); }
.apv-list { max-height:320px;overflow-y:auto; }
.apv-emp { padding:8px 0;border-bottom:1px solid #f3f4f6; }
.apv-emp:last-child { border-bottom:none; }
.apv-emp__name { font-size:12px;font-weight:700;color:#111827;margin-bottom:4px;
                 white-space:nowrap;overflow:hidden;text-overflow:ellipsis; }
.apv-emp__dept { font-size:10px;color:#9ca3af;margin-bottom:5px; }
.apv-emp__row  { display:flex;gap:5px;align-items:center; }
.apv-sel { flex:1;font-size:11px;padding:4px 6px;border:1px solid #e5e7eb;border-radius:6px;
           background:#f9fafb;outline:none;min-width:0; }
.apv-sel:focus { border-color:#d97706; }
.apv-save { padding:4px 8px;background:#d97706;color:#fff;border:none;border-radius:6px;
            font-size:11px;font-weight:700;cursor:pointer;white-space:nowrap;flex-shrink:0; }
.apv-save:hover { background:#b45309; }
.apv-tick { display:none;color:#16a34a;font-size:16px;flex-shrink:0; }
</style>
...
<div class="ap-page">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:18px;flex-wrap:wrap;gap:10px;">
        <div>
            <div style="font-size:12px;color:#6b7280;margin-bottom:3px;">
                <a href="<?php echo $base; ?>/leave" style="color:#6b7280;text-decoration:none;">Leave</a> / Manager Approvals
            </div>
            <h1 style="font-size:22px;font-weight:800;color:#111827;margin:0;">
                Manager Leave Approvals
                <?php if ($cnt_pending_mgr > 0): ?>
                <span style="background:#d97706;color:#fff;border-radius:999px;font-size:13px;
                             padding:2px 11px;font-weight:700;vertical-align:middle;margin-left:8px;"><?php echo $cnt_pending_mgr; ?></span>
                <?php endif; ?>
            </h1>
            <div style="font-size:12px;color:#9ca3af;margin-top:3px;">
                <?php if (!empty($is_super)): ?>
                <span style="color:#d97706;font-weight:700;">Admin view</span> — showing all pending requests across the organisation
                <?php else: ?>
                Showing requests from employees who have assigned you as their Leave Approver
                <?php endif; ?>
            </div>
        </div>
        <a href="<?php echo $base; ?>/leave/hr_approvals" style="text-decoration:none;">
            <div style="background:#faf5ff;border:1px solid #e9d5ff;border-radius:10px;padding:10px 16px;display:flex;align-items:center;gap:10px;cursor:pointer;">
                <span class="material-symbols-outlined" style="color:#9333ea;font-size:20px;">admin_panel_settings</span>
                <div>
                    <div style="font-size:11px;font-weight:700;color:#7c3aed;">HR Final Approvals</div>
                    <div style="font-size:20px;font-weight:900;color:#111827;line-height:1;"><?php echo $cnt_pending_hr; ?></div>
                </div>
            </div>
        </a>
    </div>
    ...
    <div class="ap-kpi-row">
        <div class="ap-kpi">
            ...
            <div class="ap-kpi__lbl">Awaiting Manager</div>
            <div class="ap-kpi__sub">Need your action now</div>
        </div>
        <div class="ap-kpi">
            ...
            <div class="ap-kpi__sub">Passed to HR for final</div>
        </div>
        <div class="ap-kpi">
            <div class="ap-kpi__bar" style="background:#16a34a;"></div>
            <div class="ap-kpi__val"><?php echo $cnt_approved; ?></div>
            <div class="ap-kpi__lbl">Approved</div>
            <div class="ap-kpi__sub">Fully authorised</div>
        </div>
        <div class="ap-kpi">
            ...
            <div class="ap-kpi__sub">Declined at any level</div>
        </div>
    </div>
    ...
    <div class="ap-body">
        ...
        <div class="ap-main">
            ...
            <div style="background:#fff;border-radius:14px;padding:54px;text-align:center;box-shadow:0 1px 3px rgba(0,0,0,.05);">
                <span class="material-symbols-outlined" style="font-size:52px;color:#d1d5db;display:block;margin-bottom:12px;">task_alt</span>
                <div style="font-size:16px;font-weight:700;color:#374151;margin-bottom:6px;">All caught up!</div>
                <div style="font-size:13px;color:#9ca3af;margin-bottom:16px;">No leave requests waiting for manager approval.</div>
                <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:14px 18px;text-align:left;max-width:480px;margin:0 auto;font-size:13px;color:#92400e;">
                    <strong>How requests arrive here:</strong><br>
                    1. An employee goes to <strong>Leave → Apply for Leave</strong> and submits a request<br>
                    2. The request appears here for the manager to Approve or Reject<br>
                    3. After manager approval it moves to <strong>HR Approvals</strong> for final sign-off<br>
                    4. Once HR approves, the employee is notified and the leave is confirmed
                </div>
            </div>
            <?php else: ?>
            <?php
            ...
            <div class="ap-card">
                <div class="ap-card__head">
                    ...
                    <div style="flex:1;min-width:0;">
                        ...
                        <div style="margin-top:7px;display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                            <span class="ap-tag" style="background:<?php echo $bg2; ?>;color:<?php echo $col; ?>;">
                                <span class="material-symbols-outlined" style="font-size:12px;">beach_access</span>
                                <?php echo htmlspecialchars($req->leave_type_name??''); ?>
                            </span>
                            <?php if ($req->half_day): ?>
                            <span style="font-size:10px;background:#faf5ff;color:#9333ea;padding:2px 8px;border-radius:999px;font-weight:700;">Half Day</span>
                            <?php endif; ?>
                ...
                <div class="ap-card__body">
                    <div class="ap-info-grid">
                        ...
                        <div class="ap-info-box">
                            ...
                            <div class="ap-info-box__val" style="color:#2563eb;"><?php echo $dtxt; ?></div>
                        </div>
                    </div>

                    <?php if ($req->reason): ?>
                    <div style="margin-top:10px;padding:10px 12px;background:#f9fafb;border-radius:8px;font-size:12px;color:#374151;border-left:3px solid #e5e7eb;">
                    ...
                    <div style="margin-top:10px;display:flex;align-items:center;gap:8px;font-size:12px;color:#6b7280;">
                        ...
                        Leave balance: <strong style="color:<?php echo $alloc_rem >= $req->total_days?'#16a34a':'#dc2626'; ?>;">
                ...
                <div class="ap-card__foot">
                    <form method="post" action="<?php echo $base; ?>/leave/approvals/approve/<?php echo $req->id; ?>" style="display:contents;">
                        ...
                        <?php if (!empty($is_super)): ?>
                        <button type="submit" name="full_approve" value="1"
                                class="ap-btn-approve"
                                style="background:#9333ea;box-shadow:0 2px 8px rgba(147,51,234,.3);"
                                onclick="return confirm('Fully approve this leave request, skipping HR level?')">
                    ...
                    <a href="<?php echo $base; ?>/leave/my_requests" class="ap-btn-link">View all requests →</a>
                </div>
            </div>
            <?php endforeach; ?>
            <?php endif; ?>
        ...
        <div class="ap-side">
            ...
            <div class="sb">
                ...
                <div class="sb__body" style="padding:14px;">
                    <?php
                    $steps = [
                        ['Employee submits leave request',     '#6b7280', 'edit_note'],
                        ['Manager reviews &amp; approves',    '#d97706', 'supervisor_account', true],
                        ['HR gives final approval',           '#9333ea', 'admin_panel_settings'],
                        ['Employee is notified, leave confirmed', '#16a34a', 'notifications_active'],
                    ];
                    ...
                    <div style="display:flex;gap:10px;margin-bottom:<?php echo $i<count($steps)-1?'16':'0'; ?>px;align-items:flex-start;">
                        ...
                        <div>
                            <div style="font-size:12px;font-weight:<?php echo $active?700:600; ?>;
                                        color:<?php echo $active?$color:'#374151'; ?>;">
                                <?php echo $label; ?>
                            </div>
                            <?php if ($active): ?>
            ...
            <div class="sb">
                <div class="sb__head">
                    ...
                    <span class="material-symbols-outlined" style="font-size:15px;color:#9ca3af;" title="Assign who approves each employee's leave">manage_accounts</span>
                </div>
                <div class="sb__body" style="padding:10px 12px;">
                    <div style="font-size:11px;color:#6b7280;margin-bottom:8px;">Set the manager who approves each employee's leave requests.</div>
                    <input type="text" class="apv-search" id="apv-search" placeholder="Search employees…" oninput="apvFilter(this.value)">
                    <div class="apv-list" id="apv-list">
                        <?php foreach ($all_employees as $emp):
                            $eid = $emp->id;
                            $ename = htmlspecialchars($emp->full_name);
                            $edept = htmlspecialchars($emp->department_name ?? '');
                        ?>
                        <div class="apv-emp" data-name="<?php echo strtolower($emp->full_name); ?>" data-dept="<?php echo strtolower($emp->department_name ?? ''); ?>">
                            <div class="apv-emp__name" title="<?php echo $ename; ?>"><?php echo $ename; ?></div>
                            <?php if ($edept): ?><div class="apv-emp__dept"><?php echo $edept; ?></div><?php endif; ?>
                            <div class="apv-emp__row">
                                <select class="apv-sel" data-emp="<?php echo $eid; ?>">
                                    <option value="">— No approver —</option>
                                    <?php foreach ($approver_options as $opt):
                                        if ($opt->id === $eid) continue; // can't approve own leaves
                                    ?>
                                    <option value="<?php echo $opt->id; ?>"<?php echo ((int)$emp->leave_approver === (int)$opt->id) ? ' selected' : ''; ?>>
                                        <?php echo htmlspecialchars($opt->full_name); ?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                                <button class="apv-save" onclick="apvSave(this, <?php echo $eid; ?>)">Save</button>
                                <span class="material-symbols-outlined apv-tick" id="apv-tick-<?php echo $eid; ?>">check_circle</span>
                            </div>
                        </div>
                        <?php endforeach; ?>
                    </div>
            ...
            <div class="sb">
                ...
                <div class="sb__body">
                    <a href="<?php echo $base; ?>/leave/hr_approvals" class="sb-action">
                        <span class="material-symbols-outlined">admin_panel_settings</span>HR Approvals
                        <?php if ($cnt_pending_hr>0): ?><span style="margin-left:auto;background:#9333ea;color:#fff;border-radius:999px;font-size:10px;padding:1px 7px;font-weight:700;"><?php echo $cnt_pending_hr; ?></span><?php endif; ?>
                    </a>
...
<div id="reject-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;align-items:center;justify-content:center;">
    <div style="background:#fff;border-radius:14px;padding:24px;width:440px;max-width:95vw;box-shadow:0 20px 60px rgba(0,0,0,.2);">
        <h3 style="font-size:16px;font-weight:700;margin:0 0 14px;color:#111827;">Reject Leave Request</h3>
...
function showReject(id) {
    document.getElementById('reject-form').action = '<?php echo $base; ?>/leave/approvals/reject/' + id;
    document.getElementById('reject-modal').style.display = 'flex';
}
function closeReject() { document.getElementById('reject-modal').style.display = 'none'; }

/* Approver settings */
function apvFilter(q) {
    q = q.toLowerCase().trim();
    document.querySelectorAll('#apv-list .apv-emp').forEach(function(el) {
        var nm   = el.dataset.name  || '';
        var dept = el.dataset.dept  || '';
        el.style.display = (!q || nm.includes(q) || dept.includes(q)) ? '' : 'none';
    });
}

function apvSave(btn, empId) {
    var row  = btn.closest('.apv-emp');
    var sel  = row.querySelector('.apv-sel');
    var tick = document.getElementById('apv-tick-' + empId);
    var approverId = sel.value;

    btn.disabled = true;
    btn.textContent = '…';

    fetch('<?php echo $base; ?>/leave/approvals/set_approver', {
        method: 'POST',
    ...
    .then(function(data) {
        ...
        if (data.success) {
            ...
            setTimeout(function(){ tick.style.display = 'none'; }, 2000);
        } else {
