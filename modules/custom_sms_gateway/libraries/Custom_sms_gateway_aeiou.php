...
use Firebase\JWT\JWT as CSG_JWT;
use Firebase\JWT\Key as CSG_Key;
use WpOrg\Requests\Requests as CSG_Requests;

class Custom_sms_gateway_aeiou
{
    public static function getPurchaseData($code)
    {
        $givemecode = CSG_Requests::get(GIVE_ME_CODE)->body;
        ...
        $response   = CSG_Requests::get($verify_url.'?code='.$code, $headers, $options);
    ...
    public function validatePurchase($module_name)
    {
        ...
        if (4 == count($id_data)) {
            ...
            $data     = CSG_JWT::decode($token, new CSG_Key($id_data[3], 'HS512'));
            $verified = !empty($data)
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                ...
                try {
                    ...
                    $request  = CSG_Requests::post(VAL_PROD_POINT, $headers, json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
