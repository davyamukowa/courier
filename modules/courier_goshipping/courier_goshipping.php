...
class Courier_Logistic_System {
    ...
    private function register_hooks() {
        ...
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v31']);
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v32']);
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v33']);
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v34']);
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v35']);
        ...
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v40']);
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v41']);
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v42']);
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v43']);
        ...
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v49']);
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v50']);
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v51']);
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v52']);
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v53']);
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v54']);
        hooks()->add_action('admin_init', [$this, 'run_db_upgrades_v55']);
...
<p>Thank you for choosing <strong>{company_name}</strong>.</p>',
    ...
    /**
     * v49: international leg statuses 12/13 grammar fix — "Arrived
     * Destination Airport" / "Arrived Go Shipping Warehouse" read as a
     * missing preposition; "Arrived at ..." is correct English.
     */
    public function run_db_upgrades_v49() {
        if (get_option('courier_schema_v49_done')) return;
        $CI = &get_instance();

        $statuses_table = db_prefix() . '_shipment_statuses';
        if ($CI->db->table_exists($statuses_table)) {
            $CI->db->where('id', 12)->update($statuses_table, ['description' => 'Arrived at Destination Airport']);
            $CI->db->where('id', 13)->update($statuses_table, ['description' => 'Arrived at Go Shipping Warehouse']);
        }

        update_option('courier_schema_v49_done', '1');
    }
