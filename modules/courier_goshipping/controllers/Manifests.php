...
class Manifests extends AdminController
{
    ...
    public function store()
    {
        ...
        $is_update = $this->Manifest_model->manifestExists($manifest_number);
        $allowed = $is_update
            ? (is_admin() || staff_can('edit_manifests', 'courier-manifests'))
            : (is_admin() || staff_can('create_manifests', 'courier-manifests'));

        if (!$allowed) {
            echo json_encode([
                'success' => false,
                'message' => 'Access denied — you do not have permission to ' . ($is_update ? 'edit' : 'create') . ' manifests.',
            ]);
            return;
        }
    ...
    public function view($manifest_number)
    {
        ...
        if (empty($data['manifest_records'])) {
            ...
            redirect(admin_url('courier_goshipping/manifests'));
            return;
        }
