...
// New private method to add recipient
    ...
    private function add_shipment($client_id, $sender_id, $recipient_id, $waybill_number, $recipient_company_id)
    {
        ...
        // TEMP DIAGNOSTIC — remove once the agent visibility issue is confirmed fixed.
        // Plain file at public_html/shipment_debug.log — open directly via File Manager.
        file_put_contents(FCPATH . 'shipment_debug.log', '[' . date('Y-m-d H:i:s') . "] CREATED id={$shipment_id} staff_id=" . get_staff_user_id() . ' stamped_branch_id=' . courier_get_session_branch_id() . ' shipping_category=' . ($shipment_data['shipping_category'] ?? 'null') . ' staff_branch_ids=' . json_encode(courier_get_staff_branch_ids()) . ' can_view_all_branches=' . (courier_staff_can_view_all_branches() ? '1' : '0') . "\n", FILE_APPEND);
