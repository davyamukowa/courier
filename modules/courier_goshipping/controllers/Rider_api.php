...
class Rider_api extends App_Controller
{
    ...
    public function __construct()
    {
        ...
        if ($this->db->table_exists(db_prefix() . '_shipment_status_history') && !$this->db->field_exists('changed_by_staff_id', db_prefix() . '_shipment_status_history')) {
            $this->db->query('ALTER TABLE `' . db_prefix() . '_shipment_status_history` ADD COLUMN `changed_by_staff_id` INT NULL DEFAULT NULL');
        }
    ...
    public function delivery_start($id)
    {
        ...
        $this->advance_shipment_status($shipment->id, 5, "Your order has been assigned to Rider {$rider_name}. He will call you when he arrives."); // in_transit / Local Delivery
    ...
    public function delivery_deliver($id)
    {
        ...
        // which silently corrupts long base64 blobs (not meant for
        // binary-ish data). Same bug/fix as Shipments::update_status() and
