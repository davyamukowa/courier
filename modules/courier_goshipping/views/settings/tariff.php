...
<div class="panel panel-default">
    ...
    <div class="panel-body">
                ...
                <div id="origin-rates-viewer" style="display:none;margin-top:16px;">
                    <div style="background:#f4f9ff;border:1px solid #d4e6f1;border-radius:6px;padding:14px 18px;">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
                            ...
                            <button class="cgs-btn cgs-btn--outline cgs-btn--sm" onclick="document.getElementById('origin-rates-viewer').style.display='none'">
                                <i class="fa fa-times"></i> Close
                            </button>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-bordered table-condensed cgs-table" style="font-size:12px;margin-bottom:0;">
                            ...
                            <div class="panel-body">
                                <form id="domestic-tariff-form">
                                    ...
                                    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                                        <button type="submit" class="cgs-btn cgs-btn--primary cgs-btn--sm">
                                            <i class="fa fa-upload"></i> Upload &amp; Apply
                                        </button>
                                        <button type="button" class="cgs-btn cgs-btn--outline cgs-btn--sm" id="domestic-template-btn" onclick="downloadDomesticTemplate()" disabled>
                    ...
                    <div class="col-md-6">
                        <div class="panel panel-default">
                            ...
                            <div class="panel-body" style="padding:0;">
                                <table class="table table-bordered table-condensed cgs-table" style="font-size:12px;margin-bottom:0;" id="domestic-cities-table">
                                    ...
                                    <tbody id="domestic-cities-body">
                                        ...
                                        <tr id="domestic-row-<?php echo htmlspecialchars($o['origin_city']); ?>">
                                            ...
                                            <td>
                                                <button class="cgs-btn cgs-btn--outline cgs-btn--sm" onclick="viewDomesticRates('<?php echo addslashes($o['origin_city']); ?>')" title="View rates">
                                                    <i class="fa fa-eye"></i>
                                                </button>
                                                <button class="cgs-btn cgs-btn--accent cgs-btn--sm" onclick="deleteDomesticTariff('<?php echo addslashes($o['origin_city']); ?>', this)" title="Delete all rates for this origin">
                ...
                <div id="domestic-rates-viewer" style="display:none;margin-top:16px;">
                    <div style="background:#fff9ec;border:1px solid #f0e0bf;border-radius:6px;padding:14px 18px;">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
                            ...
                            <button class="cgs-btn cgs-btn--outline cgs-btn--sm" onclick="document.getElementById('domestic-rates-viewer').style.display='none'">
                                <i class="fa fa-times"></i> Close
                            </button>
                        </div>
                        <div class="table-responsive">
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-condensed cgs-table" style="font-size:13px;">
                <thead style="background:#f5f5f5;">
                    <tr>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <button class="cgs-btn cgs-btn--primary cgs-btn--sm" onclick="saveZone(<?php echo $z['id']; ?>, this)">
...
<script>
var PARCEL_RATE_SAVE_URL        = '<?php echo admin_url("courier_goshipping/settings/save_parcel_rate"); ?>';
var TARIFF_UPLOAD_URL           = '<?php echo admin_url("courier_goshipping/settings/upload_tariff_csv"); ?>';
var FCL_UPLOAD_URL              = '<?php echo admin_url("courier_goshipping/settings/upload_fcl_csv"); ?>';
var TARIFF_DELETE_URL           = '<?php echo admin_url("courier_goshipping/settings/delete_tariff_rate"); ?>';
var TARIFF_ZONE_URL             = '<?php echo admin_url("courier_goshipping/settings/update_tariff_zone"); ?>';
var ORIGIN_TARIFF_UPLOAD_URL    = '<?php echo admin_url("courier_goshipping/settings/upload_origin_tariff_csv"); ?>';
var ORIGIN_TARIFF_DELETE_URL    = '<?php echo admin_url("courier_goshipping/settings/delete_origin_tariff"); ?>';
var ORIGIN_TARIFF_RATES_URL     = '<?php echo admin_url("courier_goshipping/settings/origin_tariff_rates_json"); ?>';
var ORIGIN_TARIFF_TEMPLATE_URL  = '<?php echo admin_url("courier_goshipping/settings/download_origin_tariff_template"); ?>';
var COUNTRIES_URL               = '<?php echo base_url("courier_goshipping/portal/get_countries"); ?>';
var DOMESTIC_TARIFF_UPLOAD_URL   = '<?php echo admin_url("courier_goshipping/settings/upload_domestic_tariff_csv"); ?>';
var DOMESTIC_TARIFF_DELETE_URL   = '<?php echo admin_url("courier_goshipping/settings/delete_domestic_tariff"); ?>';
var DOMESTIC_TARIFF_RATES_URL    = '<?php echo admin_url("courier_goshipping/settings/domestic_tariff_rates_json"); ?>';
var DOMESTIC_TARIFF_TEMPLATE_URL = '<?php echo admin_url("courier_goshipping/settings/download_domestic_tariff_template"); ?>';
        ...
        .then(function(res) {
            ...
            if (existing) {
                ...
                tr.innerHTML = '<td><strong>' + escHtml(origin) + '</strong></td>'
                    ...
                    + '<td>'
                    + '<button class="cgs-btn cgs-btn--outline cgs-btn--sm" onclick="viewDomesticRates(\'' + escJs(origin) + '\')" title="View rates"><i class="fa fa-eye"></i></button> '
                    + '<button class="cgs-btn cgs-btn--accent cgs-btn--sm" onclick="deleteDomesticTariff(\'' + escJs(origin) + '\', this)" title="Delete all rates for this origin"><i class="fa fa-trash"></i></button>'
                    + '</td>';
        ...
        .then(function(res) {
            ...
            res.data.forEach(function(r) {
                ...
                tr.innerHTML = '<td>' + escHtml(r.destination_city) + '</td>'
                    ...
                    + '<td><button class="cgs-btn cgs-btn--accent cgs-btn--sm" onclick="deleteDomesticRate(' + r.id + ', \'' + escJs(origin) + '\', this)"><i class="fa fa-trash"></i></button></td>';
                tbody.appendChild(tr);
            });
        })
        .catch(function() {
...
function deleteDomesticRate(id, origin, btn) {
    ...
    var fd = new FormData();
    fd.append(CSRF_TOKEN_NAME, CSRF_TOKEN_HASH);
    fetch('<?php echo admin_url("courier_goshipping/settings/delete_domestic_tariff_rate"); ?>/' + id, { method: 'POST', body: fd })
        .then(function(r){ return r.json(); })
        .then(function(res) {
            if (res.success) {
