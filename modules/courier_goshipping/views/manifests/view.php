...
<style>
    ...
    <div style="padding-left:20px; padding-right:20px;">
        <button style="margin-bottom:20px;" class="cgs-btn cgs-btn--outline" onclick="printTable()"><i class="fa fa-print"></i> Print Manifest</button>
        <button style="margin-bottom:20px;" class="cgs-btn cgs-btn--accent" onclick="downloadManifestPdf()"><i class="fa fa-file-pdf-o"></i> Download PDF</button>
        <button style="margin-bottom:20px;" class="cgs-btn cgs-btn--primary" onclick="saveManifest()"><i class="fa fa-save"></i> Update Manifest</button>
        ...
        <div class="container-wrapper">
            ...
            $_v_address   = $_ci_v['address'] ?: '';
            $_v_phone     = $_ci_v['phone']   ?: '';
            ?>
            <div class="header-container">
                <div class="logo">
                    <?php if ($_v_logo_url): ?>
                    <img width="200" height="auto" src="<?php echo $_v_logo_url; ?>" alt="<?php echo htmlspecialchars($_v_company); ?>">
                    <?php else: ?>
                    <div style="width:200px;height:80px;background:#f0f0f0;display:flex;align-items:center;justify-content:center;font-size:11px;color:#999;">No Logo</div>
                    <?php endif; ?>
                </div>
                <div style="margin-left:80px;" class="details-container">
                    <div class="details-section">
                        <h3><?php echo htmlspecialchars(strtoupper($_v_company)); ?></h3>
                        <?php if ($_v_address): ?><p><strong><?php echo nl2br(htmlspecialchars($_v_address)); ?></strong></p><?php endif; ?>
                        <?php if ($_v_phone): ?><p><strong>Tel: <?php echo htmlspecialchars($_v_phone); ?></strong></p><?php endif; ?>
                    </div>
...
<script>
    $(document).ready(function () {
        ...
        window.saveManifest = function () {
            ...
            $.ajax({
                url: '<?php echo admin_url("courier_goshipping/manifests/store"); ?>',
                type: "POST",
                data: {
                    [csrfName]: csrfHash,
                ...
                success: function (data) {
                    ...
                    window.location.href = '<?php echo admin_url('courier_goshipping/manifests') ?>'
                },
        ...
        window.downloadManifestPdf = function () {
            var btn = document.querySelector('button[onclick="downloadManifestPdf()"]');
            if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fa fa-spinner fa-spin"></i> Generating…'; }
            var el = document.querySelector('.container-wrapper');
            html2pdf().set({
                margin: [6, 6, 6, 6],
                filename: 'manifest-<?php echo date("Y-m-d"); ?>.pdf',
