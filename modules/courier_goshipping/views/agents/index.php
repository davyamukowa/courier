    ...
    <?php foreach ($agents as $agent): ?>
        ...
        <tr>
            ...
            <td class="agent-actions">
                <a href="<?php echo admin_url('courier_goshipping/agents/view/' . $agent->id); ?>" class="btn-view">
                    <i class="fa fa-eye"></i> View
                </a>
                <a href="<?php echo admin_url('courier_goshipping/agents/edit/' . $agent->id); ?>" class="btn-edit">
                    <i class="fa fa-pencil"></i> Edit
                </a>
                <?php if ($is_suspended): ?>
                    <a href="<?php echo admin_url('courier_goshipping/agents/activate/' . $agent->id); ?>" class="btn-activate" onclick="return confirm('Activate this agent?');">
                ...
                <?php endif; ?>
                <?php if (is_admin()): ?>
                    <a href="<?php echo admin_url('courier_goshipping/agents/delete/' . $agent->id); ?>" class="btn-delete" onclick="return confirm('Permanently delete this agent?');">
...
<div style="text-align:center;padding:60px 0;color:#aaa;">
    ...
    No agents yet. <a href="<?php echo admin_url('courier_goshipping/agents/main?group=create_agent'); ?>">Create the first agent</a>.
</div>
<?php endif; ?>
...
$(function () {
    $('.btn-do-suspend').on('click', function () {
        ...
        $('#suspend-form').attr('action', '<?php echo admin_url('courier_goshipping/agents/suspend/'); ?>' + $(this).data('id'));
    ...
    $('#btn-reset-role').on('click', function () {
        ...
        $.post('<?php echo admin_url('courier_goshipping/agents/reset_permissions'); ?>', {
