...
class Shipments extends AdminController
{
    ...
    public function main()
    {
        ...
        switch ($group) {
            ...
            case 'create_shipment':
                ...
                redirect(admin_url('courier_logistic/shipments/create') . ($qs ? '?' . $qs : ''));
                return;
            ...
            case 'list_shipments':
                ...
                redirect(admin_url('courier_logistic/shipments') . ($qs ? '?' . $qs : ''));
                return;
    ...
    public function filter_shipments()
    {
        ...
        if (empty($this->input->post('filterDateRange')) &&
            ...
            if ($type !== 'domestic') {
                redirect(admin_url('courier_logistic/shipments') . '?type=' . $type . '&mode=' . $mode . '&mode_type=' . $mode_type);
            } else {
                redirect(admin_url('courier_logistic/shipments') . '?type=' . $type);
            }
        }
        ...
        if (!empty($this->input->post('filterDateRange')) || $this->input->post('status_id') !== '0' || $this->input->post('staff_id') !== '0') {
            ...
            $url = 'courier_logistic/shipments?type=';
    ...
    public function store_recipient_data()
    {
        ...
        if ($recipient_id === false) {
            ...
            redirect(admin_url('courier_logistic/pickups/create'));
        }
...
// New private method to add recipient
    ...
    private function barcodeAutoloader($class)
    {
        ...
        $base_dir = FCPATH . 'modules/courier_logistic/libraries/php-barcode-generator-main/src/';
    ...
    public function generate_barcode($code)
    {
        ...
        $directory = FCPATH . 'modules/courier_logistic/assets/barcodes/';
        ...
        return base_url('modules/courier_logistic/assets/barcodes/' . $safeCode . '.png');
    }
    ...
    {
        $upload_path = FCPATH . 'modules/courier_logistic/assets/commercial_invoices/';
        ...
        if (move_uploaded_file($_FILES['commercial_invoice_file']['tmp_name'], $file_path)) {
            return 'modules/courier_logistic/assets/commercial_invoices/' . $file_name;
        } else {
    ...
    public function store()
    {
		...
		$this->session->unset_userdata('no_shipments');
                ...
                redirect(admin_url('courier_logistic/shipments/create'), $data);
    ...
    private function _assert_ownership($details)
    {
        ...
        if ((int)$details['shipment']->staff_id !== (int)get_staff_user_id()) {
            ...
            redirect(admin_url('courier_logistic/shipments/main'));
            return false;
        }
        return true;
    }
    ...
    public function assign_agent($id)
    {
        ...
        redirect(admin_url('courier_logistic/shipments/waybill/' . $id));
    }
    ...
    public function send_waybill_email($id)
    {
        ...
        $tracking_url  = base_url('courier_logistic/tracking') . '?track=' . urlencode($shipment->waybill_number ?? $shipment->tracking_id);
        ...
        try {
            ...
            if ($new_status_id === 8) {
                ...
                if (!empty($canvasData)) {
                    ...
                    $fileName = uniqid() . '.png';
                    $filePath = FCPATH . 'modules/courier_logistic/assets/deliveries/signatures/' . $fileName;
