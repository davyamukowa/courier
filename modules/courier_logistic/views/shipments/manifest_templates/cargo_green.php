...
<?php $mc = htmlspecialchars($manifest_color ?? '#2e7d32', ENT_QUOTES); ?>
<style>
/* ═══════════════════════════════════════════════════════════════════════════
...
.mf-btn-pdf    { background: linear-gradient(135deg,#6a1b9a,#4a148c); color: #fff; }
...
/* ── Form fields grid ───────────────────────────────────────────────────── */
.mf-form-grid { border-bottom: 1.5px solid; background: #fff; }
...
.mf-cell-value:focus { background: #f1f8e9; }
...
.mf-cargo-table tfoot td {
    padding: 8px 6px; font-weight: 700; color: #fff;
...
/* ── Signature footer ───────────────────────────────────────────────────── */
.mf-footer-section { border-top: 2px solid; background: #f9fbe7; padding: 10px 14px; }
.mf-footer-row  { display: flex; gap: 20px; flex-wrap: wrap; }
.mf-footer-cell { flex: 1; min-width: 160px; }
.mf-footer-label { font-size: 9px; font-weight: 700; text-transform: uppercase; display:block; margin-bottom:4px; }
.mf-sig-line    { border-bottom: 1.5px solid; min-height: 28px; margin-top: 6px; padding-top:4px; }
...
<style>
.mf-topbar         { border-bottom-color: <?php echo $mc; ?>40; }
.mf-stats-strip    { background: <?php echo $mc; ?>; }
.mf-document       { border-color: <?php echo $mc; ?>; }
.mf-co-header      { background: linear-gradient(135deg, <?php echo $mc; ?> 0%, <?php echo $mc; ?>cc 100%); }
.mf-cargo-table thead th { background: <?php echo $mc; ?>; }
.mf-cargo-table tfoot td { background: <?php echo $mc; ?>dd; }
.mf-form-grid      { border-bottom-color: <?php echo $mc; ?>; }
.mf-cell-label     { color: <?php echo $mc; ?>; }
.mf-cell-value:focus { border-bottom-color: <?php echo $mc; ?>; }
.mf-footer-section { border-top-color: <?php echo $mc; ?>; }
.mf-footer-label   { color: <?php echo $mc; ?>; }
.mf-sig-line       { border-bottom-color: <?php echo $mc; ?>; }
.mf-topbar-title   { color: <?php echo $mc; ?>; }
</style>
...
<div id="mf-wrapper">
    ...
    <div class="mf-topbar mf-no-print">
        <div class="mf-topbar-left">
            ...
            <a href="<?php echo admin_url('courier_logistic/shipments?type=domestic'); ?>" class="mf-btn mf-btn-back">
                <i class="fa fa-arrow-left"></i> Shipment List
            </a>
            <a href="<?php echo admin_url('courier_logistic/shipments/create?type=domestic'); ?>" class="mf-btn mf-btn-create">
        ...
        <div class="mf-topbar-right">
            ...
            <button class="mf-btn mf-btn-pdf" onclick="downloadManifestPdf()">
            ...
            <div class="mf-form-row">
                ...
                <div class="mf-form-cell">
                    ...
                    <span class="mf-cell-value" contenteditable="true"><?php
                        ...
                        if (!empty($modes)) { $mc2 = array_count_values($modes); arsort($mc2); echo htmlspecialchars(key($mc2)); }
                    ?></span>
                </div>
            </div>
        </div>
                ...
                ?>
                    <?php if (!empty($pod_lines)): ?>
                        ...
                        <tr<?php if (!$is_first): ?> style="background:<?php echo $pi % 2 === 0 ? '#f9fbe7' : '#fff'; ?>!important;"<?php endif; ?>>
                            ...
                            <td rowspan="<?php echo $row_span; ?>">
                                ...
                                <div style="font-size:10px; color:#555; margin-top:1px;"><i class="fa fa-phone" style="font-size:8px;"></i> <?php echo htmlspecialchars($row['sender_phone']); ?></div>
                                <?php endif; ?>
                            </td>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="font-weight:700;"><?php echo htmlspecialchars($row['sender_name']); ?></div>
                            <?php if ($row['sender_phone'] !== '-'): ?>
                            <div style="font-size:10px; color:#555;"><i class="fa fa-phone" style="font-size:8px;"></i> <?php echo htmlspecialchars($row['sender_phone']); ?></div>
                            <?php endif; ?>
                        ...
                        <td>
                            <div style="font-size:10px; color:#444;"><?php echo htmlspecialchars($row['recv_phone']); ?></div>
                            <div style="font-size:9px; font-weight:700;"><?php echo htmlspecialchars($row['waybill']); ?></div>
                        </td>
...
function sendManifestEmail() {
    ...
    $.ajax({
        url: '<?php echo admin_url("courier_logistic/shipments/send_manifest_email"); ?>',
        type: 'POST',
        data: {
...
var ROUTES_URL      = '<?php echo admin_url("courier_logistic/shipments/get_routes"); ?>';
var STOPS_URL_BASE  = '<?php echo admin_url("courier_logistic/shipments/get_route_stops"); ?>';
var SAVE_ROUTE_URL  = '<?php echo admin_url("courier_logistic/shipments/save_route"); ?>';
var DEL_ROUTE_URL   = '<?php echo admin_url("courier_logistic/shipments/delete_route"); ?>';
var SAVE_STOPS_URL  = '<?php echo admin_url("courier_logistic/shipments/save_route_stops"); ?>';
...
function downloadManifestPdf() {
    var btn = document.querySelector('.mf-btn-pdf');
    if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fa fa-spinner fa-spin"></i> Generating…'; }
    var el = document.getElementById('mf-document');
