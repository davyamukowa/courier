...
<?php echo '<link rel="stylesheet" href="' . base_url('modules/courier_logistic/assets/waybill.css') . '">'; ?>
<?php echo '<link rel="stylesheet" href="' . base_url('modules/courier_logistic/assets/progress.css') . '">'; ?>
...
<?php $this->load->view('courier_logistic/layout/_topnav', ['cgs_active' => 'shipments']); ?>
    <div class="content">
        <div class="row">
            ...
            $wb_list_url  = 'courier_logistic/shipments?type=' . $wb_type;
            if ($wb_mode) {
                ...
                <div class="courier-sidebar">
                    ...
                    <nav class="courier-sidebar-nav">
                        ...
                        <div style="padding:6px 10px; display:flex; flex-direction:column; gap:6px;">
                            <a href="<?php echo admin_url('courier_logistic/shipments/courier_invoice/' . $wb_sid); ?>"
                            ...
                            <?php if (is_admin()): ?>
                            <a href="<?php echo admin_url('courier_logistic/shipments/consignment_note/' . $wb_sid); ?>"
                               style="display:flex; align-items:center; gap:8px; background:linear-gradient(135deg,#2e7d32,#1b5e20); color:#fff; padding:8px 14px; border-radius:6px; font-size:13px; font-weight:600; text-decoration:none; transition:opacity .2s;"
                               onmouseover="this.style.opacity='.85'" onmouseout="this.style.opacity='1'">
...
$('#wb_cpm_submit').on('click', function () {
    ...
    $.ajax({
        url:      '<?php echo admin_url("courier_logistic/shipments/confirm_portal_request/" . $_wb_sid); ?>',
        type:     'POST',
        dataType: 'json',
        data: {
...
<div class="modal fade" id="update_status" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <?php echo form_open(admin_url('courier_logistic/shipments/update_status/' . $shipment_details['shipment']->id), ['id' => 'update-shipment-status-form']); ?>
...
<div class="modal fade" id="assign_agent_modal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <?php echo form_open(admin_url('courier_logistic/shipments/assign_agent/' . $wb_sid)); ?>
            <div class="modal-body">
                <div class="form-group">
...
$('#send_waybill_email_btn').on('click', function () {
    ...
    $.ajax({
        url: '<?php echo admin_url('courier_logistic/shipments/send_waybill_email/' . $shipment_details['shipment']->id); ?>',
        type: 'POST',
        dataType: 'json',
        data: { email: email },
