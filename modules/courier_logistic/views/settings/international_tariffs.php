                        ...
                        <table class="table table-bordered table-condensed cgs-table" style="font-size:13px;margin-bottom:0;" id="origin-countries-table">
                            ...
                            <tbody id="origin-countries-body">
                                ...
                                <tr id="origin-row-<?php echo htmlspecialchars($o['origin_country']); ?>">
                                    ...
                                    <td>
                                        <a href="<?php echo admin_url('courier_logistic/settings/view_origin_rates?origin=' . urlencode($o['origin_country'])); ?>" class="cgs-btn cgs-btn--outline cgs-btn--sm" style="margin-right: 5px; margin-bottom: 5px;" target="_blank">
...
<script>
const COUNTRIES_URL = '<?php echo base_url("courier_logistic/tracker/get_countries"); ?>';
...
function downloadMatrixTemplate() {
    var origin = document.getElementById('matrix_origin_country').value;
    var service = document.getElementById('matrix_service_type').value;
    window.location.href = '<?php echo admin_url("courier_logistic/settings/download_matrix_template"); ?>?origin=' + encodeURIComponent(origin) + '&service=' + encodeURIComponent(service);
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('matrix-upload-form').addEventListener('submit', function(e) {
        ...
        fetch('<?php echo admin_url("courier_logistic/settings/upload_matrix_csv"); ?>', {
            method: 'POST',
            body: formData,
...
function bulkDeleteOrigins(btn) {
    ...
    fetch('<?php echo admin_url("courier_logistic/settings/bulk_delete_origin_tariffs"); ?>', {
