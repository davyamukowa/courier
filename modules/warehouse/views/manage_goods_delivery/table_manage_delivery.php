...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, ['id','date_add','date_c','goods_delivery_code','sub_total', 'type_of_delivery', 'total_money']);
   ...
   for ($i = 0; $i < count($aColumns); $i++) {
    $CI           = & get_instance();
        ...
        if($aColumns[$i] == 'customer_code'){
            ...
            if($aRow['sub_total'] != 0){
                $_data = app_format_money((float)$aRow['sub_total'], $base_currency);
            }else{
                $_data = app_format_money((float)$aRow['total_money'], $base_currency);
            }

        }elseif($aColumns[$i] == 'after_discount'){
            $_data = app_format_money((float)$aRow['after_discount'], $base_currency);
